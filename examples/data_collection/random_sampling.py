...
def main():
    ...
    with tqdm(total = conf.sample_num) as pbar:
        while cnt < conf.sample_num:
            ...
            sample_pos = [random.uniform(-55, 3),
            ...
            if not done:
                ...
                data_record.append(np.concatenate([np.array([obs['poses_x'][0], obs['poses_y'][0], obs['poses_theta'][0]]), np.array(obs['scans'][0])[np.arange(0, 1080, 3)]]))
                # print(cnt)
                cnt += 1
                pbar.update(1)
